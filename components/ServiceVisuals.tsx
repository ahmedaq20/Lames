...
const productModes = [
  { key: 'web', label: 'Web', icon: Monitor, file: 'web.platform', metric: '99 Performance', gradient: 'from-blue-500 to-cyan-400', stack: ['Next.js', 'React', 'Tailwind', 'TypeScript'], deploy: '12s deploy' },
  { key: 'mobile', label: 'Mobile', icon: Smartphone, file: 'mobile.app', metric: 'iOS + Android', gradient: 'from-violet-500 to-purple-400', stack: ['React Native', 'Expo', 'iOS', 'Android'], deploy: 'OTA updates' },
  { key: 'api', label: 'API', icon: Database, file: 'api.service', metric: 'Secure & Scalable', gradient: 'from-emerald-500 to-cyan-400', stack: ['Node.js', 'PostgreSQL', 'REST', 'GraphQL'], deploy: '8ms latency' },
] as const
    ...
    <div className="relative flex h-[440px] w-full max-w-[560px] flex-col overflow-hidden rounded-[2rem] border border-blue-400/20 bg-slate-950 p-5 shadow-2xl shadow-blue-950/20 sm:p-6">
      ...
      >
        <div className="flex h-9 shrink-0 items-center gap-2 border-b border-white/10 px-4">
          ...
          <span className="ml-auto flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-wider text-emerald-300"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" /> Live</span>
        </div>
            ...
            <div className="space-y-3 border-r border-white/10 p-4 font-mono text-[10px] text-slate-400 sm:p-5 sm:text-xs">
              ...
              {codeLines.map((line, lineIndex) => (
                <motion.div
                  ...
                  className={`h-2 origin-left rounded ${line.width} ${line.color}`}
                />
              ))}
              <div className="flex items-center gap-1 pt-1">
                ...
                {!reduceMotion && (
                  <motion.span
                    ...
                    className="ml-1 inline-block h-3.5 w-[2px] bg-blue-300"
                  />
                )}
              </div>
            </div>
      ...
      <div className="relative mt-4 flex shrink-0 flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-wider text-slate-400">
          <span className="flex items-center gap-1.5 text-emerald-300"><Check size={13} /> Build passing</span>
...
function FinalMockup({ accent }: { accent: (typeof accents)[number] }) {
  return <div className="h-full bg-gradient-to-b from-white to-slate-50 p-4 dark:from-slate-900 dark:to-slate-950"><div className="mb-3 flex items-center justify-between"><span className="text-[9px] font-bold uppercase tracking-widest text-slate-500">Final UI</span><Sparkles size={13} className="text-violet-500" /></div><div className={`mb-3 rounded-xl bg-gradient-to-br p-4 text-white shadow-lg ${accent.gradient}`}><div className="mb-2 h-2 w-3/5 rounded bg-white/70" /><div className="h-1.5 w-4/5 rounded bg-white/30" /><div className="mt-5 h-7 w-20 rounded-full bg-white/90" /></div><div className="grid grid-cols-2 gap-2"><div className={`h-16 rounded-lg ${accent.soft}`} /><div className={`h-16 rounded-lg ${accent.soft}`} /></div></div>
}
...
function UiUxVisual() {
  ...
  return (
    <div className="relative h-[420px] w-full max-w-[560px] overflow-hidden rounded-[2rem] border border-violet-200 bg-gradient-to-br from-white via-violet-50 to-violet-100 p-5 shadow-2xl shadow-violet-200/40 dark:border-violet-400/15 dark:from-slate-950 dark:via-[#1c1024] dark:to-slate-950 sm:p-6">
      <div className="relative flex items-center justify-between gap-3">
        <div><p className="text-[10px] font-bold uppercase tracking-[0.22em] text-violet-600 dark:text-violet-300">Design playground</p><p className="text-xs text-slate-500">Compare and customize</p></div>
